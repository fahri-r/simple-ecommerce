...
@section('content')
    <div class="contain py-16">
        <div class="max-w-lg mx-auto shadow px-6 py-7 rounded overflow-hidden">
                ...
                <div class="flex items-center justify-between mt-6">
                    <div class="flex items-center">
                        <input type="checkbox" name="remember" id="remember"
                            class="text-pink-500 focus:ring-0 rounded-sm cursor-pointer">
                        <label for="remember" class="text-gray-600 ml-3 cursor-pointer">Remember me</label>
                    </div>
                    <a href="#" class="text-pink-500">Forgot password</a>
                </div>
                <div class="mt-4">
                    <button type="submit"
                        class="block w-full py-2 text-center text-white bg-pink-500 border border-pink-500 rounded hover:bg-transparent hover:text-pink-500 transition uppercase font-roboto font-medium">Login</button>
                </div>
            </form>
            ...
            <p class="mt-4 text-center text-gray-600">Don't have account? <a href="{{ route('register.index') }}"
                    class="text-pink-500">Register
                    now</a></p>
        </div>
    </div>
@endsection
...
@section('script')
    <script>
        document.querySelector('form').addEventListener('submit', function(event) {
            ...
            axios.post('{{ route('auth.login') }}', formData)
                .then(response => {
